...
export class RerouteCreateDialogComponent {
  ...
  @ViewChild('fileUpload', { static: false }) fileDropEl:
    | ElementRef
    | undefined;
  ...
  presetColors = ['#000000', '#50D139', '#1E88E5', '#D32F2F', '#F57C00'];
  ...
  createNewReroute() {
    ...
    if (
      ...
      !this.route.startsWith('https://')
    ) {
      this.route = 'https://' + this.route;
    }
    ...
    this.supabase
      .createReroute(this.name, this.route, this.color, this.imageFile)
      .then((id) => {
