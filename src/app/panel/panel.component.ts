...
export class PanelComponent implements OnInit {
  ...
  preventCreateReroute(): boolean {
    switch (this.subType) {
      case 'free':
        this.preventCreateReason = this.preventCreate
          ? 'You have reached the maximum number of reroutes for this subscription type.'
          : '';
        return this.reroutes.data.length >= 1;
      case 'basic':
        this.preventCreateReason = this.preventCreate
          ? 'You have reached the maximum number of reroutes for this subscription type.'
          : '';
        return this.reroutes.data.length >= 5;
      case 'unlimited':
        this.preventCreateReason = '';
        return false;
      default:
        this.preventCreateReason = 'Loading...';
        return true;
    }
  }
